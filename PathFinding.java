...
public class PathFinding {
	...
	private void initialize() {	//INITIALIZE THE GUI ELEMENTS
		...
		frame.setTitle("Path Finding From Start to End Point");
		...
		creditB.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				JOptionPane.showMessageDialog(frame, "	                         Pathfinding\n"
												   + "             Copyright (c) 2021\n"
												   + "                         Rakesh Arigela\n"
												   + "          Build Date:  Nov 25, 2021   ", "Credit", JOptionPane.PLAIN_MESSAGE, new ImageIcon(""));
			}
		});
...
//	------------------------------------- Algorithms that we used for finding shortest path from source point to destination point---------------------------
	...
	class Node {
		...
		public double getEuclidDist() {		//CALCULATES THE EUCLIDIAN DISTANCE TO THE FINISH NODE
			...
			dToEnd = Math.sqrt((xdif*xdif)+(ydif*ydif));   // d = sqrt((x2-x1)^2+(y2-y1)^2)
			return dToEnd;
		}
